...
def search_market_listings(  # noqa: C901
    make: str,
    model: str,
    year: int,
    ...
    **kwargs,
    ...
    # We start with the most specific query, but if it fails, we might need to retry with fewer params.  # noqa: E501
    ...
    try:
        ...
        if num_found == 0 and (exterior_color or interior_color or mileage):
            print(
                "DEBUG: No results found. Retrying with relaxed parameters (Year/Make/Model/Trim)..."  # noqa: E501
            )
            ...
            data = response.json()
            print(
                f"DEBUG: Relaxed Search Response -> Found: {data.get('num_found', 0)} listings"  # noqa: E501
            )
