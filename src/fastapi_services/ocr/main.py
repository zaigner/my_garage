...
def parse_receipt_data(text: str) -> Dict[str, Any]:  # noqa: C901
    """
    ...
    for line in lines:
        ...
        if price is not None:
            ...
            if any(keyword in line_lower for keyword in total_keywords):
                # If we find multiple "totals", we usually want the largest one (e.g. Total vs Subtotal)  # noqa: E501
            ...
            if not is_summary:
                ...
                match = re.search(
                    r"(?:\$)?\s*(\d{1,3}(?:,\d{3})*\.\d{2})(?:\s*[A-Za-z]{1,2})?$", line
                )
                if match:
...
@router.post("/process")
async def process_ocr(file: UploadFile = File(...)):  # noqa: B008
    """
