...
from ..tasks import task_collection_item_enrich, task_collection_item_refresh_valuation
...
class DynamicCollectionItemViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return DynamicCollectionItem.objects.filter(
    ...
    def refresh_valuation(self, request, pk=None):
        ...
        if not provider.supports_valuation_refresh():
            return Response(
                {"error": "Valuation refresh not supported for this collection type."},
    ...
    def enrich(self, request, pk=None):
        ...
        if not provider.supports_enrichment():
            return Response(
                {"error": "Enrichment not supported for this collection type."},
                status=status.HTTP_400_BAD_REQUEST,
            )
