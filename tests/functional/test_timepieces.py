...
/timepieces/<id>/ is a smart permalink redirect — finds the migrated DynamicCollectionItem
by source_timepiece_id and forwards to collections:collection_item_detail.
"""
import pytest
...
class TestLegacyTimepieceDetailRedirect:
    def test_migrated_timepiece_redirects_to_collection_item(
        ...
        """Legacy /timepieces/<source_timepiece_id>/ redirects to collection item detail."""
        source_id = migrated_timepiece_item.custom_fields["source_timepiece_id"]
        response = auth_client.get(f"/timepieces/{source_id}/")
        assert response.status_code == 302
        assert f"/collections/horology-salon/items/{migrated_timepiece_item.id}/" in response["Location"]
